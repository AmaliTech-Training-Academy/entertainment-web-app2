const resultsContainer = document.querySelector(".searchItems");
        ...
        resultsContainer.innerHTML = results;
            ...
            input.addEventListener('keyup', () => {
              resultsContainer.style.display = "grid" 
               ...
               if (input.value === "") {
               document.querySelector(".videos").style.display = "grid"; 
               resultsContainer.style.display = "none" 
                ...
                for(i = 0; i < filterResult.length; i++){
                  ...
                  //  +'<img class="search-item-img" src="'+ filterResult[i].thumbnail.regular.medium+ '"/>'
    ...
    function searchByName(title) {
           ...
           for (i = 0; i < data.length; i++) {
                  if (data[i].category=== 'TV Series' && data[i].title.toLowerCase().includes(title.toLowerCase())) {
                    results.push(data[i]);
                }
            }
            return results;
